...
const UsagePage: React.FC = () => {
    ...
    return (
        <div className="min-h-screen flex flex-col items-center p-4 bg-gradient-to-br from-gray-900 to-black font-inter text-stone-800">
            ...
            <header className="text-center mb-12 mt-8">
                <h1 className="text-4xl md:text-5xl font-bold text-teal-800">アプリ操作ガイド</h1>
                <p className="mt-2 text-lg text-stone-600">対話的に操作を学ぶ</p>
            </header>

            <main className="w-full max-w-5xl mx-auto">
                {/* Section 1: UI Tour */}
                <section id="ui-tour" className="mb-16">
                    <h2 className="text-3xl font-bold mb-6 text-center text-teal-700">画面要素の探検</h2>
                    <p className="text-center text-stone-600 mb-8 max-w-3xl mx-auto">
                        下のアプリ画面の模型にカーソルを合わせると、各要素の説明が表示されます。静的なリストを読むよりも直感的に、どの部分がどのような機能を持つのかを理解できます。
                    </p>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                            ...
                            <div className="md:col-span-1 bg-stone-100 p-4 rounded-lg flex flex-col gap-4">
                                <div className="tooltip-container">
                                    <LiquidGlassButton className="w-full bg-teal-600 hover:bg-teal-700 text-white py-2 rounded-lg flex items-center justify-center gap-2">
                                ...
                                <div className="tooltip-container">
                                    <input type="text" placeholder="再生時間(秒)" className="w-full p-2 border border-stone-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:outline-none text-stone-800" />
                                    <div className="tooltip">時間入力フィールド: 和音図の再生時間を秒単位で設定します。</div>
                                </div>
                                <div className="tooltip-container">
                                    <select className="w-full p-2 border border-stone-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:outline-none text-stone-800">
                                        <option>Sine</option>
                                        <option>Sawtooth</option>
                                        <option>Square</option>
                                        <option>Triangle</option>
                                    </select>
                                    <div className="tooltip">シンセサイザータイプ選択: サウンドの波形タイプを選択します。</div>
                                </div>
                                <div className="tooltip-container mt-4 border-t border-stone-300 pt-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        <LiquidGlassButton className="bg-white hover:bg-stone-200 text-stone-800">2D</LiquidGlassButton>
                                        <LiquidGlassButton className="bg-white hover:bg-stone-200 text-stone-800">2D ▼</LiquidGlassButton>
                                        <LiquidGlassButton className="bg-white hover:bg-stone-200 text-stone-800">3D</LiquidGlassButton>
                                        <LiquidGlassButton className="bg-white hover:bg-stone-200 text-stone-800">3D ▼</LiquidGlassButton>
                                        <LiquidGlassButton className="bg-white hover:bg-stone-200 text-stone-800">4D</LiquidGlassButton>
                                        <LiquidGlassButton className="bg-white hover:bg-stone-200 text-stone-800">4D ▼</LiquidGlassButton>
                                        <LiquidGlassButton className="bg-white hover:bg-stone-200 text-stone-800">5D</LiquidGlassButton>
                                        <LiquidGlassButton className="bg-white hover:bg-stone-200 text-stone-800">5D ▼</LiquidGlassButton>
                                    </div>
                                    <div className="tooltip">次元ボタン: 音符の次元を変更したり、ピッチを変更したりするために使用されます。</div>
                                </div>
                            </div>
                            {/* Right Panel - Score Canvas Mock */}
                            <div id="score-canvas-mock" className="md:col-span-3 relative bg-gray-700 rounded-lg p-4 h-80 overflow-hidden flex items-center justify-center">
                                {/* Simple visual mock-up of the canvas content */}
                                <div className="absolute inset-0 flex items-center justify-center">
                                    <div className="w-full h-full relative">
                                        {/* Center line */}
                                        <div className="absolute left-0 right-0 h-px bg-stone-400" style={{ top: '50%' }}></div>
                                        {/* Other lines */}
                                        {Array.from({ length: 4 }).map((_, i) => (
                                            <React.Fragment key={`line-up-${i}`}>
                                                <div className="absolute left-0 right-0 h-px bg-stone-500" style={{ top: `calc(50% - ${40 * (i + 1)}px)` }}></div>
                                                <div className="absolute left-0 right-0 h-px bg-stone-500" style={{ top: `calc(50% + ${40 * (i + 1)}px)` }}></div>
                                            </React.Fragment>
                                        ))}
                                        {/* Mock notes */}
                                        <div className="absolute bg-white rounded-full w-5 h-5" style={{ left: '50px', top: 'calc(50% - 10px)' }}></div>
                                        <div className="absolute bg-white/80 rounded-md w-32 h-2" style={{ left: '80px', top: 'calc(50% - 5px)' }}></div>
                                        <div className="absolute bg-white/80 rounded-md w-32 h-2" style={{ left: '220px', top: 'calc(50% - 40px - 5px)' }}></div>
                                    </div>
                                </div>
                ...
                <section id="editing-guide" className="mb-16">
                    ...
                    <div className="w-full max-w-4xl mx-auto">
                        ...
                        {/* Tab Content */}
                        <div id="tab-content" className="bg-white p-6 rounded-lg shadow-inner border border-stone-200 min-h-[300px] text-left">
                            {activeTab === 'tab1' && (
                                    ...
                                    <ul className="space-y-4">
                                        {tabContent.tab1.items.map((item, index) => (
                                            <li key={index} className="flex items-start gap-4">
                                                <div className="flex-shrink-0 w-8 h-8 bg-teal-100 text-teal-700 rounded-full flex items-center justify-center font-bold">{index + 1}</div>
                                                <div>
                                                    <h4 className="font-semibold">{item.title}</h4>
                                                    <p className="text-stone-600" dangerouslySetInnerHTML={{ __html: item.description }} />
                                                </div>
                                            </li>
                                        ))}
                                    </ul>
                            ...
                            {activeTab === 'tab2' && (
                                <div id="tab2" className="tab-panel">
                                    ...
                                    <div className="flex flex-col items-center">
                                        <p className="mb-4 text-stone-600" dangerouslySetInnerHTML={{ __html: tabContent.tab2.description }} />
                                        <div className="flex items-center justify-center space-x-2 mb-4">
                                            <span id="mode-label-1" className={`font-semibold ${!isPitchMode ? 'text-teal-700' : 'text-stone-500'}`}>子音符追加モード</span>
                                            <label htmlFor="mode-toggle" className="relative inline-flex items-center cursor-pointer">
                                                ...
                                                />
                                                <div className="w-11 h-6 bg-stone-300 rounded-full peer peer-checked:bg-teal-600 transition-colors"></div>
                                                <div className="absolute left-1 top-1 w-4 h-4 bg-white rounded-full transition-transform peer-checked:translate-x-full"></div>
                                            </label>
                                            <span id="mode-label-2" className={`font-semibold ${isPitchMode ? 'text-teal-700' : 'text-stone-500'}`}>ルート音ピッチモード</span>
                                        </div>
                                        <div id="mode-description" className="w-full bg-stone-100 p-4 rounded-lg text-center" dangerouslySetInnerHTML={{ __html: tabContent.tab2.modeDescription }} />
                                    </div>
                                </div>
                            )}
                            {activeTab === 'tab3' && (
                                <div id="tab3" className="tab-panel">
                                    ...
                                    <p className="text-stone-600 mb-4" dangerouslySetInnerHTML={{ __html: tabContent.tab3.description }} />
                                    <div className="grid md:grid-cols-2 gap-6">
                                        {tabContent.tab3.modes.map((mode, index) => (
                                            <div key={index} className="bg-stone-100 p-4 rounded-lg">
                                                ...
                                                <p className="text-sm text-stone-600" dangerouslySetInnerHTML={{ __html: mode.explanation }} />
            ...
            <footer className="text-center mt-16 border-t border-stone-200 pt-6 w-full max-w-5xl mx-auto">
                <p className="text-sm text-stone-500">このインタラクティブガイドは、提供されたソースコードに基づいて生成されました。</p>
            </footer>
        </div>
    );
};
